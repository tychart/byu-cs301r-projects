...
query the vector database to help answer the user's question
        ...
        query_agent = ChatAgent("gpt-5-nano", query_agent_sysprompt)
        
        query_request = await query_agent.get_response(message)

        print(f"Query Agent: {query_request}")
